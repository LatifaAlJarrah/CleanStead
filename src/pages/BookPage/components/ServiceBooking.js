  ...
  const BookingDate = () => {
    ...
    return (
      <Formik
        ...
        onSubmit={(values) => {
          localStorage.setItem("selectedService", values.serviceRepetition);
          localStorage.setItem("selectedDate", values.selectedDate);
        }}
      >
        {({ values }) => (
            ...
            <div>
              ...
              <div className="flex">
                <label className="booking-date p-3 rounded-xl color-booking-date flex w-72 h-12 items-center justify-between">
                  {values.selectedDate
                    ? new Date(values.selectedDate).toLocaleDateString(
                        "en-US",
                        {
                          year: "numeric",
                          month: "long",
  ...
  useEffect(() => {
    const calculateTotalPrice = (options) => {
      ...
      options.forEach((option) => {
        if (option.services.service1) {
          calculatedTotalPrice += option.services.service1.price;
        }
        if (option.services.service2) {
          calculatedTotalPrice += option.services.service2.price;
        }
      });
    ...
    const storedOptions =
      JSON.parse(localStorage.getItem("selectedCleaningOptions")) || [];
    setTotalPrice(calculateTotalPrice(storedOptions));

    const intervalId = setInterval(() => {
      const updatedOptions =
        JSON.parse(localStorage.getItem("selectedCleaningOptions")) || [];
      setTotalPrice(calculateTotalPrice(updatedOptions));
    }, 1000);
  ...
  return (
    <div className="grid grid-cols-12 xl:gap-20 md:gap-10 mt-6 justify-between">
      ...
      <div className="col-span-8 bg-container">
        ...
        {currentStep === 3 && <UserInformation onContinue={handleContinue} />}
