  ...
  return (
    <nav>
      <div
        ...
        <a
          href="/"
          className="flex columns-2 no-underline">
          ...
          <span
            className="text-xl font-semibold ml-1 clean-text-color"
          >كلين
          </span>
        ...
        <div
          ...
          <div className="relative lg:hidden">
...

            {
              isOpenHomePage && (
                <ul
                  className="list-none absolute top-12 left-2 w-48 p-4 shadow-lg rounded-lg bg-white z-10">
                  {
                    linkes.map((link, index) => (
                      <li
                        className="py-2 cursor-pointer no-underline"
                        key={index}>
                        {
                          link.name
                        }
          ...
          <ul class="list-none flex flex-col mt-4 lg:flex-row lg:mt-0 w-96 h-8 justify-between">
            {
              linkes.map((link, index) => (
                ...
                >
                  <a href={link.linkPage} className="leading-8 text-center text-base items-list-color item-list no-underline">
                    {link.name}
                  </a>
                </li>
              ))}
